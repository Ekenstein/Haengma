...
namespace Haengma.Backend.Functional.Sgf
{
    public class SgfReader
    {
        ...
        private static Parser<char, Sign> NumberSign { get; } = Char('+')
            .Or(Char('-'))
            .Select(v => v == '+' ? Sign.Plus : Sign.Minus);

        private static Parser<char, (Sign? sign, int number)> Number { get; } =
            (from sign in Try(NumberSign).Optional().Select<Sign?>(x => x.HasValue ? x.Value : null)
             from value in Int
             select (sign, value))
            .Labelled("Number");

        private static Parser<char, (Sign? sign, double number)> Real { get; } =
        ...
        private static Parser<char, SgfProperty> ToProperty(string identifier) => identifier switch
        {
            ...
            "KM" => PropertyValue(Real).Select<SgfProperty>(x => new KM(x.number)),
